...
create_var_map <- function () {
  ...
  var_map[['educ_bach_major_2_digit']] <- tribble(
    ~year, ~code,
    2019, 'ER34737',
    2021, 'ER34937',
    2023, 'ER35137'
...
saveRDS(var_map, file = here::here('data/PSID/clean/var_map.rds'))
var_map <- readRDS(file = here::here('data/PSID/clean/var_map.rds'))

expand_year_var_map <- function (var_map) {
  var_year <- list()
  
  for (year in 1990:2023) {
    var_list <- c()
    for (list in 1:length(var_map)) {
      if (length(var_map[[list]]$year) == 1) {
        var_list <- c(var_list, var_map[[list]]$code)
      } else {
        var_list <- c(var_list,
                      var_map[[list]] %>%
                        filter(year == !!year) %>%
    ...
    var_year[[as.character(year)]] <- var_list
  }
  
  return (var_year)
}

expand_var_maps <- function (var_map) {
  for (list in 1:length(var_map)) {
    var_map[[list]]$varname <- names(var_map[list])
    var_map[[list]]$year <- as.character(var_map[[list]]$year)
  }
  
  var_maps <- bind_rows(var_map)
  
  return (var_maps)
}

var_maps <- expand_var_maps(var_map)
saveRDS(var_maps, file = here::here('data/PSID/clean/var_maps.rds'))
